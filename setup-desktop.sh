...
configure_openclaw_desktop() {
    ...
    # Create OpenClaw config (JSON format)
...
EOF
    ...
    # Set DISPLAY in .env (OpenClaw loads this early)
    cat > "${OPENCLAW_CONFIG_DIR}/.env" << EOF
...
EOF
    ...
    # Also set DISPLAY in shell profiles for interactive use
    grep -q "export DISPLAY=:1" "${OPENCLAW_HOME}/.bashrc" 2>/dev/null || \
        echo 'export DISPLAY=:1' >> "${OPENCLAW_HOME}/.bashrc"
    grep -q "export DISPLAY=:1" "${OPENCLAW_HOME}/.profile" 2>/dev/null || \
        echo 'export DISPLAY=:1' >> "${OPENCLAW_HOME}/.profile"
    
    log_success "OpenClaw configured for desktop mode (headless: false, DISPLAY=:1)"
}
